...
def main(model_pth, km_pth, save_pth, mean_std_pth, data_pth, tsv_pth):
    ...
    with open('./info/dev-clean-speaker.json') as fp:
        speaker_dict = json.load(fp)  
        num_spk = 0 
        for k, v in speaker_dict.items():
            num_spk = max(num_spk, v)
        num_spk += 1
    ...
    for pth in tqdm(wav_pths): 
        ...
        spk_key = key.split('-')[0].strip()
        spk = speaker_dict[spk_key] 
        ...
        for layer_idx, (fc1, fc2) in enumerate(fc_results):
            ...
            for k, indices in enumerate(topk_indices):
                c = clusters[k]
                record[layer_idx][spk*N_cluster+c, indices] += 1
                record_n[layer_idx][spk*N_cluster+c] += 1
